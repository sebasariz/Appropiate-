...
function loadView(grid) {
    ...
    for (var i = 0; i < grid.length; i++) {
        ...
        var result = "<div class=\"row\"> " +
                ...
                "                <div class=\"search-item-thumbnail\"><img src=\"img/" + evento.img + "\" alt=\"\" class=\"img-responsive\"></div>" +
                "                <div class=\"search-item-detail\">" +
                "                    <a href=\"#\" class=\"search-item-heading font-header\">" + evento.nombre + "</a>" +
                "                    <p class=\"search-item-description font-12\">" + evento.informacion + "</p>" +
                "                    <div class=\"row m-t-15\">" +
                "                        <div class=\"col-sm-5\">" +
                "                            <div class=\"font-semi-bold \">" +
                "                               Interesados: <span class=\"text-main\">" + evento.numeroPosibles + "</span> " +
                "                            </div>" +
                "                        </div><!-- /.col -->" +
                ...
                "                        <div class=\"col-sm-5\">" +
                "                            <div class=\"font-semi-bold \">" +
                "                                 Asistentes: <span class=\"text-main\">" + evento.numeroAsistentes + "</span>" +
                "                            </div>" +
                "                        </div><!-- /.col -->" +
                "                        <div class=\"col-sm-7 text-right text-left-xs\">" +
                "                            <button type=\"button\" class=\"btn btn-gray btn-icon\" onclick=\"eliminarEvento(" + evento.id + ")\"><i class=\"icon-bin\"></i></button>" +
                "                            <button type=\"button\" class=\"btn btn-dark\" onclick=\"LoadEventoJson(" + evento.id + ")\">Editar</button>" +
                "                            <button type=\"button\" class=\"btn btn-main\" onclick=\"LoadChat(" + evento.id + ")\">Comentarios</button>" +
                "                        </div><!-- /.col -->" +
                "                    </div><!-- /.row -->" +
                "                </div>" +
                "            </div><!-- /.serach-item -->" +
...
$("#gestion").submit(function (e) {
    ...
    if ($("#gestion").prop("accion") == 'crear') {
        action = 'CrearEvento.appropiate';
    } else if ($("#gestion").prop("accion") == 'editar') {
        action = 'EditarEvento.appropiate';
    }
    $.ajax({
        type: "POST",
        cache: false,
